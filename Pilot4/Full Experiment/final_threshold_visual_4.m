%% This script runs the second visual threshold part of the fourth pilot.
...
DataFile = [WorkingDir '\1.3-WP4-' p.SubjectsNumber '_Run_' p.SessionNr '_final_visual_threshold']; %set folders per subject
      ...
      %% Update the pdf
        if mod(randomQuestPick,2) == 1
           ...
           if contrast <= 0.001
                ...
                %% Remove the quest and circle from their respective array, only if contrast is not smaller equal 0.001
                if contrast > 0.001
...
fprintf(FID, '\r\n');
fprintf(FID, '\r\n');
fprintf(FID, '%6s %8s %9s %10s \r\n','Trial','Contrast','Answer','Quest Mode');
fprintf(FID, '%6.0f %8.5f %9s %10s \r\n',A);
%fprintf(FID, '%s \r\n',answersTrialArray{:});
fprintf(FID, '\r\n');
