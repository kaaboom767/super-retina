%% This script runs the tRNS threshold part of the fourth pilot.
...
DataFile = [WorkingDir '\1.3-WP4-' p.SubjectsNumber '_Run_' p.SessionNr '_tRNS_threshold']; %set folders per subject
...
%% Check if flickering during high frequency stimulation was perceived and if yes, print out in which trial
if any(strcmp(tRNS_hf_Array,'flickering perceived'))
    HF_Index = find(contains(tRNS_hf_Array,'flickering perceived'));
    HF_Flickering = ['High frequency flickering in Trial(s) ' (mat2str(HF_Index*2)) ' perceived!'];
else
