%% This script runs the visual threshold part of the third pilot.
...
maxBoundary = 1;
trial = 1;
dummyTrials = sort(randperm((numTrials - 4),4));
...
DataFile = [WorkingDir '\1.3-WP3-' p.SubjectsNumber '_Run_' p.SessionNr '_estimated_visual_threshold']; %set folders per subject
...
%     coin = 0;
    ...
    if  not (ismember(trial, dummyTrials)) 
        ...
        if  contrast <= 0.001
            ...
            %% Remove the current circle from their respective array
...
fprintf(FID, '\r\n');
fprintf(FID, '\r\n');
fprintf(FID, '%6s %8s %6s \r\n','Trial','Contrast','Answer');
fprintf(FID, '%6.0f %8.5f %6s \r\n',A);
%fprintf(FID, '%s \r\n',answersTrialArray{:});
fprintf(FID, '\r\n');
fprintf(FID, 'Percentage answered with yes: %f',percentageCorrect);
fprintf(FID, '%%');
fclose(FID);
