%% This script runs the experiment part of the third pilot.
...
WorkingDir = ['C:\Users\kaaboom\Hyperion Cloud\ETH\Master Thesis\Data\Pilot3\1.3-WP3-' p.SubjectsNumber '\']; %set directory for windows to save data
WorkingDir = [WorkingDir{:}];
WorkingDir = convertCharsToStrings(WorkingDir);

DataFile = [WorkingDir '1.3-WP3-' p.SubjectsNumber '_Run_' p.SessionNr '_experiment']; %set folders per subject
...
A = [trialArray; keyAnswerArray; answersTrialArray; intervalArray; controlInterventionArray; circleTestArray];
...
fprintf(FID, '\r\n');
fprintf(FID, '%6s %6s %11s %14s %10s %15s \r\n','Trial','Answer','Correctness','Gabor Interval','tRNS Mode','Circle Location');
fprintf(FID, '%6.0f %6s %11s %14.0f %10s %15s \r\n',A);
fprintf(FID, '\r\n');
...
B = transpose(A);
table = array2table(B,'VariableNames',{'Trial','Answer','Correctness','Gabor Interval','tRNS Mode','Circle Location'});
