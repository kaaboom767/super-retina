% This script runs the second visual threshold part of the third pilot.
...
maxBoundary = 1;
trial = 1;
dummyTrials = sort(randperm((numTrials - 4),8));
dummyArray = [];
...
DataFile = [WorkingDir '\1.3-WP3-' p.SubjectsNumber '_Run_' p.SessionNr '_final_visual_threshold']; %set folders per subject
...
%Find and extract the standard deviation to use (20)
sd20 = fileread(visualThresholdFile);
expr = '[^\n]*Standard Deviation 20: [^\n]*';
sd20 = regexp(sd20,expr,'match');
sd20 = convertCharsToStrings(sd20);
sd20 = sscanf(sd20, 'Standard Deviation 20: %u%f');
tGuessSd = sd20(2,1);
...
q120 = QuestCreate(tGuess120,tGuessSd,pThreshold,beta,delta,gamma,[],range);
...
q80 = QuestCreate(tGuess80,tGuessSd,pThreshold,beta,delta,gamma,[],range);
...
%   coin = 0;
    ...
    % check if dummy trial, if yes set contrast to 0
    if ismember(trial, dummyTrials)
        contrast = 0;
    end
    ...
    Screen('TextFont', window, 'Noto Font');
    DrawFormattedText(window, 'Did you see the gabor patch? \n \n <-- no  |  yes --> ', 'center', 'center', white);
    Screen('Flip', window);
    ...
    if keyCode(escapeKey)
        ...
        Screen('CloseAll');
        return
    elseif keyCode(leftKey) %no
        keyAnswer = 1;
        response = 0;
        answer_name = "no";
        respToBeMade = false;
        
    elseif keyCode(rightKey) %yes
        keyAnswer = 2;
        response = 1;
        answer_name = "yes";
    ...
    %% Check if dummy trial, only do the following stuff if current trial is a regular trial
    if  not (ismember(trial, dummyTrials)) 
      ...
      %% Update the pdf
        if mod(randomQuestPick,2) == 1
           ...
           if contrast <= 0.001
                ...
                q120 = QuestUpdate(q120,contrast,response);

           else

                %% Update the pdf
                q120 = QuestUpdate(q120,contrast,response); % Add the new datum (actual test intensity and observer response) to the database.
      ...
      elseif mod(randomQuestPick,2) ==0
            ...
            if contrast <= 0.001
                ...
                response = 0;
                answer_name = "incorrect";
                q80 = QuestUpdate(q80,contrast,response);

            else

                %% Update the pdf
                q80 = QuestUpdate(q80,contrast,response); % Add the new datum (actual test intensity and observer response) to the database.
    ...
    if ismember(trial, dummyTrials)
        if response == 0
            dummyArray = [dummyArray, "no"];
        elseif response == 1
            dummyArray = [dummyArray, "yes"];
        end
...
tMean = mean([t120, t80]);
sdMean = mean ([sd120, sd80]);

nCorrect = nnz(strcmp(answersTrialArray,"yes"));
percentageCorrect = nCorrect/(q120.trialCount+q80.trialCount)*100;

trialArray = (1:1:(length(contrastArray)));


%% Check if dummy trials were answered with yes
if any(strcmp(dummyArray,'yes'))
    dtIndex = find(contains(dummyArray,'yes'));
    dummyTotal = ['Dummy trials answered with yes in trial(s) ' mat2str(dummyTrials(dtIndex)) '!'];
else
    dummyTotal = ('No dummy trials answered with yes!');
end


%% Replace all dummy trials with NaN
contrastArray(contrastArray == 0) = NaN;


%% Combine all the important values into one array
A = [trialArray; contrastArray; answersTrialArray; questModeArray];
...
fprintf(FID, '\r\n');
fprintf(FID, '%s',dummyTotal);
fprintf(FID, '\r\n');
fprintf(FID, '\r\n');
fprintf(FID, '%6s %8s %6s %10s \r\n','Trial','Contrast','Answer','Quest Mode');
fprintf(FID, '%6.0f %8.5f %6s %10s \r\n',A);
%fprintf(FID, '%s \r\n',answersTrialArray{:});
fprintf(FID, '\r\n');
...
B = transpose(A);
table = array2table(B,'VariableNames',{'Trial','Contrast','Answer','Quest Mode'});
...
figure()
ix120=(q120.intensity~=0);  %keep the desired indices
ix80 = (q80.intensity~=0);

plot(q120.intensity(ix120)) % plot only those particular ones
hold on
plot(q80.intensity(ix80))

%plot(q120.intensity(1:40));

%plot(q80.intensity(1:40));
