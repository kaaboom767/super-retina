...
WorkingDir = ['C:\Users\kaaboom\Hyperion Cloud\ETH\Master Thesis\Data\Pilot2\1.3-WP2-' p.SubjectsNumber '\']; %set directory for windows to save data
WorkingDir = [WorkingDir{:}];
WorkingDir = convertCharsToStrings(WorkingDir);

DataFile = [WorkingDir '1.3-WP2-' p.SubjectsNumber '_Run_' p.SessionNr '_experiment']; %set folders per subject
...
%     currentTrial = 15;
    ...
    if (20<currentTrial) && (currentTrial<41)
        ...
        correctkey = 37;   
            ...
            %no gabor, add 40ms
            for i = 1:((2*circleWind)+round(hertz*0.04))
                Screen('FrameOval',window, black, CircleRect, 2);
                Screen('DrawLines', window, allCoords,...
                lineWidthPix, white, [xCenter yCenter], 2);
                Screen('Flip', window);
